...
import org.agrfesta.sh.api.persistence.jdbc.entities.anAssignmentEntity
...
@Import(SensorsAssignmentsDaoJdbcImpl::class)
@ActiveProfiles("test")
class AssignmentsControllerUnitTest(
    ...
    @Autowired @MockkBean private val assignmentsRepository: AssignmentsJdbcRepository,
    ...
    @Test fun `assignSensorToArea() returns 500 when device assignment fetch fails`() {
        ...
        every { assignmentsRepository.findByDevice(deviceUuid) } returns PersistenceFailure(Exception(failure)).left()
    ...
    @Test fun `assignSensorToArea() returns 500 when assignment persistence fails`() {
        ...
        val deviceUuid = sensorDeviceEntity.uuid
        every { devicesJdbcRepository.getDeviceById(deviceUuid) } returns sensorDeviceEntity.right()
        every { assignmentsRepository.findByDevice(deviceUuid) } returns
                emptyList<AssignmentEntity>().right() // no previous assignments
        every { assignmentsRepository.persistAssignment(any(), any()) } returns
    ...
    fun `assignSensorToArea() successfully assigns device, assigned to another area in the past but not anymore`() {
        ...
        val deviceUuid = sensorDeviceEntity.uuid
        every { devicesJdbcRepository.getDeviceById(deviceUuid) } returns sensorDeviceEntity.right()
        val assignmentEntity = anAssignmentEntity(
        ...
        every { assignmentsRepository.findByDevice(deviceUuid) } returns listOf(assignmentEntity).right()
        val areaUuidSlot = slot<UUID>()
        val deviceUuidSlot = slot<UUID>()
        every { assignmentsRepository.persistAssignment(capture(areaUuidSlot), capture(deviceUuidSlot)) } returns
