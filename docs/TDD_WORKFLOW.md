...
* **State over interactions (Chicago school):** At outbound ports prefer **in-memory fakes** and assert on the **resulting state**, not on calls (`verify`) against mocks. Interaction verification is legitimate **only when the interaction itself is the contract** — the wire protocol of a transport adapter (request bodies/headers/sequences observed at the HTTP boundary), or a port whose only effect is the call itself (e.g. a notification dispatcher). **Non-interaction assertions** ("method X was never called") are forbidden unless the absence of the call is an explicit domain requirement.
