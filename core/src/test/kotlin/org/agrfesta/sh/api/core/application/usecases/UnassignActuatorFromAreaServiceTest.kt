...
class UnassignActuatorFromAreaServiceTest {
    ...
    private val sut = UnassignActuatorFromAreaService(areasRepository, devicesRepository, actuatorsAssignmentsRepository)
    ...
    @Test fun `execute() returns Right(Unit) on success`() {
        ...
        every { actuatorsAssignmentsRepository.unassign(area.uuid, device.uuid) } returns Unit.right()
    ...
    @Test fun `execute() returns ActuatorNotAssigned when no assignment found for this area`() {
        ...
        val device = anActuator()
        every { devicesRepository.getDeviceById(device.uuid) } returns device.right()
        every { actuatorsAssignmentsRepository.unassign(area.uuid, device.uuid) } returns ActuatorNotAssigned.left()
